...
开发路径(01-05)之外，本 Agent 同步支持测试路径：**用例设计 → Demo 生成 → 黑盒验证**。两条路径均以 `01-analysis-prd.md` 为公共输入；测试路径产物写入同一 `.ohos-adaptation/` 目录。
...
`primary-sdk-test-design` 为测试设计主 Agent，编排 21→22→23 三个 SubAgent；`primary-sdk-demo-gen` 独立执行阶段 24。
