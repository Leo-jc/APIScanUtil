...
# 或作为模块
python -m hoscanner --project <path> --out <dir> [--ets-src ohos_library/src/main/ets] [--ai]
```

| 参数 | 说明 | 默认值 |
|------|------|--------|
| `--project` | 目标工程 / 三方库根目录 | `D:\Projects\APIScanUtil\thirdLibrary\FluidLayout\ohos_hardemo` |
...
python run_testset_local.py

# 覆盖率校验（召回率 Recall，匹配键 (api, codeLocation)，产出 coverage_report.json/md）
python tests/compute_coverage.py
python tests/coverage_to_md.py

# 快速对比（逐项目 matched/missing/extra，产出 comparison_aggregate.json）
python tests/compare_change_report.py
...
覆盖率口径详见 `output/coverage_report.md`。
...
│   ├── report.py           # JSON / Markdown 报告生成（含 C++ 专属报告）
│   ├── ai_advisor.py       # AI 升级建议（LLM 可选，确定性回退）
│   └── scanner.py          # 扫描编排主流程
├── run_scanner.py          # 命令行入口
├── run_testset.py          # 测试集批量扫描驱动
├── run_testset_local.py    # 测试集批量扫描驱动（本地路径）
├── tests/                  # 测试与覆盖校验脚本
