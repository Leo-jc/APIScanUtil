...
REM  Paths containing spaces are supported. Use DEVECO_HOME, DEV_DIR, or TOOL_HOME when auto-detection fails.
REM
...
REM Build classpath with directory wildcards to avoid exceeding cmd.exe command length.
set "CP=%DEVECO%\lib\*"
for /D %%D in ("%DEVECO%\plugins\*") do (
    if exist "%%D\lib" set "CP=!CP!;%%D\lib\*"
)

REM Compile if source is newer than class, or BUILD id changed (源码头的 // BUILD: <id> 与 .build-stamp 不一致)
set "SRC=%SCRIPTDIR%\ApiChangeCli.java"
set "CLS=%SCRIPTDIR%\ApiChangeCli.class"
set "STAMP=%SCRIPTDIR%\.build-stamp"
set "BUILDID="
for /f "tokens=3" %%B in ('findstr /R /C:"^// BUILD: " "%SRC%"') do set "BUILDID=%%B"
set "CUR="
if exist "%STAMP%" set /p "CUR="<"%STAMP%"
if not exist "%CLS%" goto :compile
if not "!CUR!"=="!BUILDID!" goto :compile
for %%A in ("%SRC%") do set "SRCTIME=%%~tA"
for %%A in ("%CLS%") do set "CLSTIME=%%~tA"
REM 时间格式 yyyy/MM/dd HH:mm 字典序可比, 仅源码比 class 新才重编 (避免每次运行都重编)
if "!SRCTIME!" gtr "!CLSTIME!" goto :compile
goto :run
